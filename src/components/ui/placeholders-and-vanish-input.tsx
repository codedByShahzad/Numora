...
export function PlaceholdersAndVanishInput({
  ...
  const draw = useCallback(() => {
    ...
    newDataRef.current = points.map(({ x, y, color }) => ({
      x,
      y,
      r: 1,
    ...
    const animateFrame = (pos: number = 0) => {
      requestAnimationFrame(() => {
        ...
        if (ctx) {
          ...
          newDataRef.current.forEach((pt) => {
            const { x, y, r, color } = pt;
            if (x > pos) {
              ctx.beginPath();
              ctx.rect(x, y, r, r);
              ctx.fillStyle = color;
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // ✅ no-unused-expressions fix (explicit call)
    onSubmit(e);
  };
      ...
      <input
        onChange={(e) => {
          if (!animating) {
            ...
            onChange(e); // ✅ no-unused-expressions fix
          }
        }}
